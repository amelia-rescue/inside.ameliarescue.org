...
export async function loader({ params, context }: Route.LoaderArgs) {
  ...
  const reminders = await reminderStore.getRemindersByUser(userId);
  const certification_data = await getCertificationData(userId);

  return { user, certification_data, reminders };
}
...
export default function User() {
      ...
      <div className="card-body gap-3 p-4">
        ...
        {certType.existing_cert?.file_url ? (
          <a
            ...
            className="btn btn-xs btn-info self-start"
          >
            View
          </a>
        ) : null}
  ...
  const renderCertRow = (certType: CertRow) => (
    <tr key={certType.name}>
      ...
      <td>{formatDateValue(certType.existing_cert?.issued_on)}</td>
      <td>{formatDateValue(certType.existing_cert?.expires_on)}</td>
